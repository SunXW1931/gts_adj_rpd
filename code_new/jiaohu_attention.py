...
class jiaohu_SelfAttention(torch.nn.Module):
    ...
    def forward(self, query, value, mask):
        ...
        attention_states_T = attention_states_T.permute([0, 2, 1])  # 改动value，添加rpd

        weights=torch.bmm(attention_states, attention_states_T)  # torch.bmm(a,b)中a的size为(b,h,w),b的size为(b,w,m),输出为(b,h,m)
