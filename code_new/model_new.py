...
from attention_module_new import MultiHeadedAttention, SelfAttention
from jiaohu_attention_new import MultiHeadedAttention, jiaohu_SelfAttention
...
class MultiInferRNNModel(torch.nn.Module):
    def __init__(self, gen_emb, domain_emb, args):
        ...
        self.gen_embedding.weight.data.copy_(gen_emb)  # 指定预训练的权重
        ...
        self.rp_embedding=torch.nn.Embedding(2*args.max_relative_position+1,args.lstm_dim*2)

        self.dropout1 = torch.nn.Dropout(0.3)
        self.dropout2 = torch.nn.Dropout(0.1)

        self.bilstm = torch.nn.LSTM(300+100+100, args.lstm_dim,
                                    num_layers=1, batch_first=True, bidirectional=True)  # batch优先
    ...
    def _get_rp_embedding(self,rp):
        rp_embed=self.rp_embedding(rp)
        rp_embed=self.dropout1(rp_embed)
        return rp_embed
    ...
    def forward(self, sentence_tokens, sentence_poses, adjs, rp, lengths, mask):
        ...
        a = lstm_feature
        lstm_feature_gcn = self.gcn_layer(lstm_feature, adjs, mask[:, :lengths[0]])
        # 添加相对位置距离
        rp_embedding=self._get_rp_embedding(rp)

        b = lstm_feature_gcn

        jiaohu_num = 3
        for i in range(jiaohu_num):
            lstm_feature_jiaohu_a = self.jiaohu_attention(lstm_feature, lstm_feature,rp_embedding, mask[:, :lengths[0]])  # 添加相对位置距离
            lstm_feature_gcn_jiaohu_a = self.jiaohu_attention(lstm_feature_gcn, lstm_feature_gcn,rp_embedding, mask[:, :lengths[0]])  # 添加相对位置距离
            ...
            lstm_feature_gcn = self.gcn_layer(lstm_feature_gcn_jiaohu_drop2, adjs, mask[:, :lengths[0]])
            # lstm_feature_gcn2 = self.gcn_layer(lstm_feature_gcn_jiaohu_drop2, adjs, mask[:, :lengths[0]])
            # lstm_feature_gcn = self.gcn_layer(lstm_feature_gcn2, rpds, mask[:, :lengths[0]])  # 添加相对位置距离
        lstm_feature = a + lstm_feature
        lstm_feature_attention = self.attention_layer(lstm_feature, lstm_feature, rp_embedding, mask[:, :lengths[0]])  # 添加rpd
