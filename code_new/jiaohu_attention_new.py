...
class jiaohu_SelfAttention(torch.nn.Module):
    def __init__(self, args):
        super(jiaohu_SelfAttention,self).__init__()
        self.args = args
    ...
    #             if distance_mat[i][j] < -max_relative_position:
    #                 distance_mat[i][j] = -max_relative_position
    #             elif distance_mat[i][j] > max_relative_position:
    #                 distance_mat[i][j] = max_relative_position
    #     final_mat = distance_mat + max_relative_position
    #     return final_mat

    # def generate_relative_positions_embeddings(self,length,d,max_relative_position):
    #     relative_positions_matrix=self.generate_relative_positions_matrix(length,max_relative_position)
    #     vocab_size=max_relative_position*2+1
    #     vocab_embedding=torch.nn.Embedding(vocab_size,d)
    #     rp_embed=vocab_embedding(relative_positions_matrix)
    #
    #     return rp_embed

    def forward(self, query, value,rp, mask):  # 添加rpd
        ...
        weights1 = torch.bmm(attention_states, attention_states_T)

        # query点乘rpd
        attention_states_q_T=query.permute([1,0,2])
        attention_rp_T=rp.permute([0,2,1])
        weights2=torch.bmm(attention_states_q_T,attention_rp_T)
        weights2=weights2.permute([1,0,2])
        # weights相加
        weights=weights1+weights2
        weights = weights.masked_fill(mask.unsqueeze(1).expand_as(weights)==0, float("-inf"))  # mask中为0的地方全都设为无穷大
